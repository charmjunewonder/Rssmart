...
@implementation ECDateUtility

+ (NSDate *)dateFromInternetDateTimeString:(NSString *)dateString formatHint:(ECDateFormatHint)hint {
	
	NSDate *date;
	
	if (hint != ECDateFormatHintRFC3339) {
		
		// try RFC822 first
		date = [ECDateUtility dateFromRFC822String:dateString];
		
		if (!date) {
			date = [ECDateUtility dateFromRFC3339String:dateString];
		}
